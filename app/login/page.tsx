...
import { loginUser, registerUser } from '@/lib/dash';
...
export default function LoginPage() {
  ...
  const submit = async () => {
    ...
    try {
      if (tab === 'login') {
        ...
        localStorage.setItem('altrodash_user', JSON.stringify(u));
        ...
        localStorage.setItem('altrodash_user', JSON.stringify(r));
        window.dispatchEvent(new Event('altrodash:refresh'));
        router.push('/');
      }
    } catch (e: any) {
  ...
  return (
    <main className="bj-form-wrap">
      <div className="bj-form-title">Altro<span>DashBoard</span></div>
      <div className="bj-form-sub">나만의 데이터를 한눈에</div>
      ...
      {tab === 'login' && (
        <div className="bj-notice" style={{ textAlign: 'center' }}>
          AltroBoard · AltroShop · AltroTodo 계정으로도 로그인할 수 있습니다.
        </div>
      )}
    </main>
  );
